...
SOFTWARE_NP=(linux-image-generic-hwe-22.04 intel-gpu-tools aspnetcore-runtime-8.0 nginx)
...
function configure_nginx {
    if [[ ! -d /etc/nginx/conf.d/remuxer ]]; then
...
EOF
    ...
    if systemctl is-active --quiet nginx; then
        ...
        whiptail --title "Nginx Configuration" --msgbox "Nginx has been configured and restarted successfully." 8 50
    else
        echo "Failed to restart Nginx. Please check the configuration."
        whiptail --title "Nginx Configuration Error" --msgbox "Failed to restart Nginx. Check the configuration and try again." 8 50
        exit 1
    fi
}
...
function deploy_np_ux {
    tar -xzf dist/np-ux.tar.gz -C /opt
    cd /opt/np-ux && ./run.sh
}
...
function install_hardware_acc {
    ...
    if dpkg --compare-versions "$CURRENT_KERNEL" lt "$TARGET_KERNEL"; then
        if whiptail --title "Kernel Update Confirmation" --yesno "Your current kernel version is $CURRENT_KERNEL. Do you want to update?" 15 60; then
            ...
            whiptail --title "Reboot Confirmation" --yesno "Kernel updated successfully. Reboot now?" 15 60 && sudo reboot
        fi
    fi
...
function install_intel_gpu_runtime {
    ...
    if [[ ! -f "$BASE_DIR/dist/MediaStack.tar.gz" ]]; then
        whiptail --title "Missing File" --msgbox "MediaStack.tar.gz not found in the dist directory. Please ensure the file exists." 10 60
        return 1
    fi
...
function remove_all {
    if whiptail --title "Remove Software" --yesno "This will remove the following software packages:\n${SOFTWARE_FFMPEG[*]}\n${SOFTWARE_NP[*]}\nDo you want to proceed?" 20 70; then
        sudo apt purge -y "${SOFTWARE_FFMPEG[@]}" "${SOFTWARE_NP[@]}"
        sudo apt autoremove -y
        sudo apt clean
        sudo rm -rf /opt/np-ux
